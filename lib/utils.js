...
var escapeICSDescription = function escapeICSDescription(description) {
  var replaceTag = description.replace(/(<a ?\/?>)/g, "");
  return replaceTag.replace(/(\r?\n|<br ?\/?>|<a ?\/?>)/g, "\\n");
};
/**
 ...
 * @param {array} event.attendees
 * @param {string} event.descriptionGoogle
 * @returns {string} Google Calendar Event URL
 */

exports.escapeICSDescription = escapeICSDescription;

var googleParsAttendeesMail = function googleParsAttendeesMail(attendees) {
  if (attendees.length === 0) {
    return "";
  }
  let str = "&";
  attendees.forEach((element) => {
    str = str.concat("add=", encodeURIComponent(element), "&");
  });
  return str;
};

var googleShareUrl = function googleShareUrl(_ref) {
  var parceAttendees = googleParsAttendeesMail(_ref.attendees);
  var description = _ref.descriptionGoogle,
    endDatetime = _ref.endDatetime,
    location = _ref.location,
    startDatetime = _ref.startDatetime,
    timezone = _ref.timezone,
    title = _ref.title,
    attendees = parceAttendees;
  return "https://calendar.google.com/calendar/render?action=TEMPLATE&dates="
    .concat(startDatetime, "/")
    .concat(endDatetime)
    .concat(timezone && "&ctz=".concat(timezone), "&location=")
 ...
 * @param {string} event.title
 * @param {array} event.attendees
 * @param {string} event.descriptionIcal
 * @returns {array} ICS Content
 */

var shareFileParsAttendeesMail = function shareFileParsAttendeesMail(
  attendees
) {
  if (attendees.length === 0) {
    return "";
  ...
  var str = "";
  attendees.forEach((element, index) => {
    if (index === attendees.length - 1) {
      str = str.concat("ATTENDEE:MAILTO:", element);
    } else {
      str = str.concat("ATTENDEE:MAILTO:", element, "\n");
    }
  });
...
var buildShareFile = function buildShareFile(_ref3) {
  var parceAttendees = shareFileParsAttendeesMail(_ref3.attendees);
  var _ref3$description = _ref3.descriptionIcal,
  ...
  var content = [
    ...
    "METHOD:PUBLISH", // TODO: Will need to parse the date without Z for ics
    // This means I'll probably have to require a date lib - luxon most likely or datefns
    timezone === ""
      ? "DTSTART:".concat(startDatetime)
      : "DTSTART;TZID=".concat(timezone, ":").concat(startDatetime),
    timezone === ""
      ? "DTEND:".concat(endDatetime)
      : "DTEND;TZID=".concat(timezone, ":").concat(endDatetime),
    "SUMMARY:".concat(title),
    "DESCRIPTION:".concat(escapeICSDescription(description)),
...
var buildShareUrl = function buildShareUrl(_ref4, type) {
  var _ref4$description = _ref4.description,
    ...
    _ref4$descriptionIcal = _ref4.descriptionIcal,
    descriptionIcal =
      _ref4$descriptionIcal === void 0 ? "" : _ref4$descriptionIcal,
    _ref4$descriptionGoogle = _ref4.descriptionGoogle,
    descriptionGoogle =
      _ref4$descriptionGoogle === void 0 ? "" : _ref4$descriptionGoogle,
    ...
    _ref4$attendees = _ref4.attendees,
    attendees = _ref4$attendees === void 0 ? [] : _ref4$attendees;

  var encodeURI =
    type !== _enums.SHARE_SITES.ICAL && type !== _enums.SHARE_SITES.OUTLOOK;
  var data = {
    ...
    attendees: attendees,
    descriptionIcal: descriptionIcal,
    descriptionGoogle: encodeURIComponent(descriptionGoogle),
  };
